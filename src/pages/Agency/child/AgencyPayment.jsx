...
const AgencyPaymentStyles = styled.div`
  display: flex;
...
const AgencyPayment = () => {
  ...
  const {
    ...
    payment_1_of_month,
    payment_16_of_month,
    money_payment_request,
  } = info;
  function requestPayment() {
    if (Number(balance) <= Number(payment_limit)) {
      dispatch({
        type: c.CHANGE_POPUP,
        popupType: c.AUTOHIDE_POPUP,
        messageInfo: `Chưa đạt hạn mức thanh toán`,
      });
      return;
    }
  ...
  useEffect(() => {
    if (account.status === c.LOADING) return;
    setPaymentAuto(account.payment_auto);
  }, [account.payment_auto, account.status]);
  useEffect(() => {
    dispatch(a.getAccountInfo());
  }, [dispatch]);
  return (
    <AgencyPaymentStyles className="payment ">
        ...
        <div className="payment-policy__request">
          {!has_payment_request ? (
            ...
            <button style={{ background: "#b8b8b8", cursor: "initial" }}>
              ...
              {formatPrice(money_payment_request ?? 0)}
            </button>
          )}
          <label>
