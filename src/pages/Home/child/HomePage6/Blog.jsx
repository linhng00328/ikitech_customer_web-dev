    ...
    if (data.id) {
      var cate = [
        <Link to={
          ...
          } title={data.title} className="article-cate uppercase">
          {data.title}
        </Link>
      ];
    } else {
      ...
      <div className="container">
        <div className="row">
          ...
          <div className="col-12 col-md-6">
                    ...
                    <Link to={
                      v.title
                        ? `/${v.post_url
                          .replace(/[^a-zA-Z ]/g, "")}`
                        : `/${v.id}`
                      } title={v.title} className="object-fit-img img">
                        ...
                        <Link to={
                          v.title
                            ? `/${v.post_url
                              .replace(/[^a-zA-Z ]/g, "")}`
                            : `/${v.id}`
                          } title={v.title} className="article-title">
                          {v.title}
                        </Link>
                      </h3>
    								</div>
            ...
            <Link to="/tin-tuc" title="Xem thêm bài viết khác" className="read-more">Xem thêm bài viết khác <i className="fa fa-angle-double-right"></i></Link>
          </div>
                      ...
                      <Link to={
                        v.title
                          ? `/${v.post_url
                            .replace(/[^a-zA-Z ]/g, "")}`
                          : `/${v.id}`
                        } title={v.title} className="article-img">
                        <img src={v.image_url} alt={v.title} />
                      </Link>
                    </div>
                    <div className="col-8">
                      <div className="article-info">
                        ...
                        <div className="article-date">
                          <i className="fa fa-calendar" aria-hidden="true" /> {moment(v.created_at).format("DD/MM/YYYY")}
                        </div>
                        {renderCategory(v)}
                      </div>
                    </div>
